// Screen-space Lens Flare overlay.
//
// Ported from Anderson Mancini's "Ultimate Lens Flare" (vanilla three.js,
// CC0): https://github.com/ektogamat/R3F-Ultimate-Lens-Flare and the vanilla
// port at https://lensflare-threejs-vanilla.vercel.app/
//
// It is rendered as its own fullscreen, additively-blended pass AFTER the main
// scene, so it never touches the gallery's existing render path (low risk). The
// flare originates from a light source in screen space (NDC) — here, the
// currently selected painting projected to the screen, or the mouse pointer.
//
// Differences from the upstream effect (kept intentionally simple + portable):
//   - No EffectComposer/postprocessing dependency: a single ShaderMaterial on a
//     fullscreen clip-space quad in a dedicated scene.
//   - `texture()` calls are written as `texture2D()` and the tonemapping/
//     encoding includes are dropped so it runs as a GLSL ES 1.00 shader on the
//     project's renderer without version juggling.
//   - The lens-dirt sampler is a small procedural noise texture (no external
//     image fetch / CORS), used only by the optional Star Burst.
...
const fragmentShader = /* glsl */ `
  ...
  float saturate2(float x){ return clamp(x, 0.,1.); }
  ...
  vec3 drawflare(vec2 p, float intensity, float rnd, float speed, int id){
    ...
    if(anamorphic){ internalStarPoints = 1.0; } else { internalStarPoints = starPoints; }
  ...
  float glare(vec2 uv, vec2 pos, float size){
    vec2 main;
    if(animated){ main = rotateUV(uv-pos, iTime * 0.1); } else { main = uv-pos; }
  ...
  vec3 LensFlare(vec2 uv, vec2 pos){
    ...
    vec2 prot;
    if(animated){ prot = rotateUV(uv - pos, (iTime * 0.1)); }
    else if(anamorphic){ prot = rotateUV(uv - pos, 1.570796); }
    else { prot = uv - pos; }
  ...
  float rnd(float w){ return fract(sin(w)*1000.); }

  float regShape(vec2 p, int N){
    float a=atan(p.x,p.y)+.2;
    float b=6.28319/float(N);
    return smoothstep(.5,.51, cos(floor(.5+a/b)*b-a)*length(p.xy)* 2.0  -ghostScale);
  }
  ...
  void main(){
    ...
    gl_FragColor = vec4(finalColor * opacity, 1.0);
  }
`;
...
// Small procedural lens-dirt texture so the Star Burst has something to sample
// without fetching an external image (avoids CORS + an extra network request).
function makeDirtTexture() {
  ...
  for (let i = 0; i < size * size; i++) {
    // Mostly dark with sparse brighter specks, like dust on glass.
    const base = 30 + Math.random() * 40;
    const speck = Math.random() > 0.985 ? 150 + Math.random() * 105 : 0;
...
export class LensFlareOverlay {
  constructor() {
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        lensDirtTexture: { value: makeDirtTexture() }
      },
      vertexShader,
      fragmentShader,
      transparent: true,
    ...
    this.camera = new THREE.Camera(); // clip-space quad; camera is unused
    this.quad = new THREE.Mesh(new THREE.PlaneGeometry(2, 2, 1, 1), this.material);
  ...
  // Whether the overlay should be drawn this frame.
  get active() {
  ...
  setEnabled(on) {
    ...
    this.material.uniforms.enabled.value = !!on;
  }
  ...
  // Source position in NDC (-1..1). Used when followMouse is off.
  setSourceNDC(x, y) {
    if (!this.followMouse) this.material.uniforms.lensPosition.value.set(x, y);
  }

  // Advance animation + (optionally) track the mouse, then draw additively over
  // the current framebuffer without clearing the gallery render beneath it.
  render(renderer, elapsed) {
    if (!this.active) return;
    this.material.uniforms.iTime.value = elapsed;
    if (this.followMouse) {
      this.material.uniforms.lensPosition.value.copy(this._mouse);
    }
  ...
  dispose() {
    ...
    const dirt = this.material.uniforms.lensDirtTexture.value;
    if (dirt && dirt.dispose) dirt.dispose();
  }
}
