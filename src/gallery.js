...
import { packWall, paintingSizeM } from './wallPack.js';
...
export class Gallery {
  ...
  // Lay paintings around a rectangular hall. Each wall uses salon-style packing:
  // every work keeps its real dimensions, gaps are edge-to-edge minimums, and
  // large/small pieces are shuffled by size tier to fill the wall densely.
  build(paintingData, layout) {
    ...
    const walls = [
      { dir: new THREE.Vector3(1, 0, 0), pos: new THREE.Vector3(0, 0, 0), rot: 0 },
      { dir: new THREE.Vector3(-1, 0, 0), pos: new THREE.Vector3(0, 0, 0), rot: Math.PI },
      { dir: new THREE.Vector3(0, 0, 1), pos: new THREE.Vector3(0, 0, 0), rot: Math.PI / 2 },
      { dir: new THREE.Vector3(0, 0, -1), pos: new THREE.Vector3(0, 0, 0), rot: -Math.PI / 2 }
    ];
    ...
    const packs = wallChunks.map((chunk) => {
      ...
      return packWall(items, hGap, vGap);
    });
    ...
    this.layout = { ...layout, perWall, hGap, vGap };

    this._buildEnvironment(wallLen, wallHeight);

    const half = wallLen / 2;
    walls[0].pos.set(0, 0, -half);
    walls[1].pos.set(0, 0, half);
    walls[2].pos.set(-half, 0, 0);
    walls[3].pos.set(half, 0, 0);

    for (let w = 0; w < walls.length; w++) {
      const wall = walls[w];
      const pack = packs[w];
      for (const slot of pack.placed) {
